...
class PiStream_server {
    ...
    start_feed() {
        ...
        //TODO: support dynamic start of stream
    }
    ...
    new_client(socket) {
        var user_limit = (this.options.limit > 0)? this.options.limit : 0;
        var condition = (user_limit === 0) ? true : this.ws_server.clients.size <= user_limit;
        var self = this;
        if(condition) {
            ...
            socket.on('close', () => {
                ...
                if(self.options.dynamic) {
                    if(self.ws_server.clients.size == 0 && self.streamer != null)
                        self.stop_feed();
                }
            });
